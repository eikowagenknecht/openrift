 ...
 * then link all card_sources with the given name to the new card.
...
export async function acceptNewCardFromSources(
  ...
  // Link all card_sources with this name to the new card
  await linkUnmatchedSources(trx, cardFields.name, cardFields.id);
}

/**
 * Set card_id on all unmatched card_sources with the given name,
 * and create a name alias.
 */
export async function linkUnmatchedSources(
  ...
  // Create alias so future uploads match automatically
  await trx
