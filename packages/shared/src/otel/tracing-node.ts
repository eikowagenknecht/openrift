/**
 * OpenTelemetry SDK bootstrap, shared by the API and the web server. Call
 ...
 *
 * When `OTEL_EXPORTER_OTLP_ENDPOINT` is unset the SDK is not started; tracer
 ...
 *
 * Node-only: it pulls `@opentelemetry/sdk-node`, so the web app must keep it
 * behind a server-only import.
 */
...
/**
 * Start the SDK and register shutdown flushing.
 *
 * @param serviceName - Fallback `service.name` when `OTEL_SERVICE_NAME` is unset.
 * @returns The started SDK, or undefined when `OTEL_EXPORTER_OTLP_ENDPOINT` is unset.
 */
