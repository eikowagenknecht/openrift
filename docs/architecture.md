...
└── Dockerfile            # Multi-stage build (api, web, migrate)
```
...
Vite + React 19 single-page app. No router — all navigation state lives in URL query parameters via nuqs. The app is installable as a PWA with offline image caching.
...
Lightweight [Hono](https://hono.dev/) server on [Node.js](https://nodejs.org/). Uses [Kysely](https://kysely.dev/) as a type-safe query builder with [PostgreSQL](https://www.postgresql.org/). Exposes a REST API consumed by the frontend.

See [Data Layer](data-layer.md) for endpoints and schema.
...
│     ▼                                                  │
│  ┌─────────────────────────┐                           │
│  │ Hono (tsx --watch)      │  apps/api (live reload)   │
│  │ :3000                   │                           │
│  └──┬──────────────────────┘                           │
│     │                                                  │
│     ▼                                                  │
...
│  │     ▼                                                      │  │
│  │  ┌─────────────────────────┐                               │  │
│  │  │ api (node:22-alpine)    │  Hono - API server (apps/api) │  │
│  │  │ :3000                   │                               │  │
│  │  └──┬──────────────────────┘                               │  │
│  │     │                                                      │  │
│  │     ▼                                                      │  │
