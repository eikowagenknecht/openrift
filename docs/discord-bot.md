...
- **`/tradechannel enabled:<true|false>`** — marks the current channel as a trade channel, where ordinary posts get scanned for card names (see "Trade channels" below). Same registration and permission rules as `/link`
...
The bot authenticates these privileged reads with `DISCORD_BOT_API_SECRET`, a shared service secret between the API and the bot (see `.env.example`; compose passes it to both containers). When it is unset, the API's `/api/v1/discord-bot/*` endpoints refuse every call, the bot skips the lookups, and neither `/link` nor `/tradechannel` is registered. Card lookups are not cached per guild — every one is an internal API round trip, and any failure just drops the field from the reply.

## Trade channels

A card lookup only helps someone who knows the bot exists. In a **trade channel** it works the other way round: an ordinary post is scanned for card names, and if anyone in the linked group offers one, the bot replies underneath with who has what. Nobody has to type a command or know the feature is there.

`/tradechannel enabled:true` marks the current channel (Manage Server permission, and the guild must already be linked). The setting is a `trade_channel_ids` array on the guild's link row (migration 222) rather than a table of its own — the scoping unit is the server, and one linked row already exists per guild. Empty is the default, so linking a server scans nothing until a channel is opted in. The bot holds the whole guild→channels map in memory (`TradeChannelCache`, refreshed every 30 minutes, written through by `/tradechannel`): whether to scan is asked of every message in every channel, which no per-message query could carry. A failed refresh keeps the previous map, and an empty map means no scanning rather than scanning everything.

Matching (`trade-scan.ts`) is deliberately exact. A card is found only when its **whole name** is written out, or when a printing code appears (`OGN-202`, `OGN-202/298` — the most precise thing a trade post can carry). No prefixes, no fuzzy distance: in a channel where people discuss League champions all day, a bare "Jinx" must not resolve to a card called "Jinx, Rebel". Both the names and the message go through the site's `foldForSearch`, then every separator becomes a token break, so punctuation the writer drops or adds never decides a match ("jinx rebel", "Jinx, Rebel" and "Jinx,Rebel." are the same thing, and "quick draw" finds Quick-Draw). A longest-match scan consumes what it matched, so the fuller name wins over a shorter one nested inside it. Code blocks, inline code, links, quoted lines, Discord entities, and `[[card name]]` references are blanked first — the last of those because the mention path already answers them, and scanning them again would reply twice.

The safety net is that **the reply is gated on supply, not on matches**: a card nobody offers contributes no line, and a message whose every name draws a blank produces no reply at all. So the noise floor is the group's actual tradelist contents, which is small, rather than the number of names a matcher happened to recognise. Replies are capped at five cards, are plain text rather than card embeds, and suppress all mentions.

`DISCORD_TRADE_SCAN_MODE` decides what happens with a match. Unset (the default, and anything other than the exact string `reply`) is **log-only**: the bot writes the reply it would have posted to its log and posts nothing. That is how a channel's real traffic tunes the matcher before the bot ever speaks unprompted — turn it to `reply` once the logs look right. A typo in the variable can only ever leave it quieter, never louder.
...
- The trade-channel map comes from `GET /api/v1/discord-bot/trade-channels` on startup and every 30 minutes, into a third cache (`TradeChannelCache`) kept separate for the same reason as the rules cache.
...
- Trade-channel scanning runs on the same message event as `[[card name]]`, before it and independently: a post in a trade channel that also carries a bracketed reference gets both, since one is an explicit lookup and the other is the offers list. Bot messages are skipped for both.
