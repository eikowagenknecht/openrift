...
- **`/card name:<card>`** — slash command with autocomplete over all card names
...
**Local dev** (uses the dev application's token):

```bash
# .env: DISCORD_BOT_TOKEN=<dev app token>
bun run --cwd apps/discord-bot dev
```

`API_INTERNAL_URL` and `SITE_URL` default to `http://localhost:3000` and `http://localhost:5173`, so a running `bun dev:api` is all it needs.
...
- Prices shown are for the card's representative printing (first canonical-rank printing with a front image), in each marketplace's own currency (TCGplayer USD, Cardmarket/CardTrader EUR).
