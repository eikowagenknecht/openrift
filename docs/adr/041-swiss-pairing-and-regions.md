...
Pairing runs the existing local-search engine with an all-twos decomposition. Rematch avoidance is the engine's dominant penalty (100/500/2000 per repeat meeting versus 10 per point of score spread), so the customer's complaint is solved by construction: the search reaches across score groups before repeating a matchup. Swiss skips the round-1 random shortcut so the region term is honored from the first round; pod mode skips it too whenever any player carries a region. A repeated-region term (`repeatedRegionWeight`, 25 per prior meeting with the opponent's region) gives players region variety across rounds: it loses to a live region mirror (70) and to any rematch, but beats moderate score spread. The history is derived on read from the opponents' current regions, like every other aggregate. An odd field auto-byes the player with the fewest byes, then lowest score (organizer byes still work; re-roll inherits the stored bye).
...
- Deferred, deliberately: deriving region suggestions from a participant's submitted deck (`deck_check_entry_cards.resolved_card_id` joined through `card_custom_tags` via `customTagsRepo.assignmentsForCardIds`). The repeated-opposing-region history term was initially deferred here and landed shortly after as `repeatedRegionWeight`.
