...
Files are organized by set, with filenames derived from the printing ID. The printing ID is a composite of `{source_id}:{art_variant}:{is_signed}:{is_promo}:{finish}` (generated by `buildPrintingId()`). For file paths, colons are replaced with hyphens and boolean flags are abbreviated to `y`/`n`:

`{set_id}/{source_id}-{art_variant}-{signed:y|n}-{promo:y|n}-{finish}-{size}.webp`

```plaintext
card-images/
└── ARC/
├── OGN/
│   ├── OGN-027-normal-n-n-foil-orig.png     # original (archived)
│   ├── OGN-027-normal-n-n-foil-300w.webp
│   ├── OGN-027-normal-n-n-foil-400w.webp
│   ├── OGN-027-normal-n-n-foil-full.webp
│   ├── OGN-027a-altart-n-n-foil-300w.webp   # alt-art
│   ├── OGN-223-normal-y-n-foil-300w.webp    # signed
│   └── ...
├── SFD/
```
