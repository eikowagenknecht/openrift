...
Files are organized by set, with filenames derived from the printing ID. The printing ID is a composite of `{source_id}:{rarity}:{finish}:{promo|}` (generated by `buildPrintingId()`). For file paths, colons are replaced with hyphens and the promo boolean is abbreviated to `y`/`n`:

`{set_id}/{source_id}-{rarity}-{finish}-{promo:y|n}-{size}.webp`
