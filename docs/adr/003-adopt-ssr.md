...
We migrate to TanStack Start. SEO became a real requirement (public card pages, social embeds), the framework matured to where the build pipeline and deployment patterns were stable enough to commit, and the Hono API can stay standalone for any future mobile client.
...
- Bad, because TanStack Start is at 1.x but effectively RC quality, and the API surface is still shifting (Nitro v2 → v3, upcoming "Vite-native" mode).
...
- Bad, because server functions are not callable from external clients, so a future mobile app would need the API extracted back out or kept alongside.
