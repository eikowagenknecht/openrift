...
- Matching itself stays simple: current card-identity match logic (ADR-013) is good enough, and preferences are display-only.
...
- **Computed prices from `marketplace_product_prices`**: show users a resolved EUR/USD number on each match row. Rejected: language-conflated prices would mislead, and users would trust the number and skip the marketplace check.

## Decision Outcome

We add a list-level default with per-entry override. Both sides carry preferences, matching is unchanged, and the values are display-only.
...
- Good, because match rows carry enough context to start a negotiation. The "what do you want for it?" round-trip dies.
- Good, because list defaults cover ~all of a typical list, with per-entry override for the outliers without forcing it.
- Good, because matching logic is untouched, so the perf surface and SQL stay identical to ADR-013.
- Good, because currency stays at list level only. A list is one transaction context, and one currency is correct for nearly all cases.
- Bad, because it adds four columns to `lists`, three to `list_entries`. Mitigated by all being nullable with a clear "no preference" semantics.
- Bad, because preference labels can drift from on-marketplace reality (CM raises prices, user's "CM lowest" intent unchanged). Acceptable: the label is the intent, not the number, and the linked page is the source of truth.
- Bad, because currency mismatches (one side EUR, other USD) are user-visible but not auto-resolved. Acceptable for v1. If it becomes a real friction we can add a hover-time conversion later.
