...
- Existing primitives (`lists`, `list_entries`, copies) cover the data. We only need group membership and a sharing scope
- Trade negotiation happens externally (Discord, in person), with no in-app messaging in this ADR
...
We build friend groups with opt-in sharing. Per-list ACLs scale poorly (N users × M lists, so any roster change churns membership) and share-token re-use gives us no matchmaking surface. A group entity captures the social unit explicitly, which is what the match view needs as its scope.

### Consequences

- Good, because matchmaking is a single SQL query: intersect my wishes with co-members' shared trades, scoped to one group.
- Good, because sharing a list with a group is one row, revoking is one row, and there is no ACL fan-out across users.
- Good, because leaving or being kicked auto-revokes via FK cascade, with no separate cleanup logic.
- Bad, because three new tables and a small surface of new endpoints. Mitigated by the model being narrow and read-mostly.
- Bad, because slugs introduce a global namespace and a squatting risk. Mitigated by a length floor and a reserved-slug list.
...
`UNIQUE (group_id, user_id)` enforces "at most one pending invite or request per (group, user)": it both deduplicates clicks and prevents duplicate-row spam.
...
- Sharing is per group, all members, live. Any subsequent edit to the list propagates immediately, with no snapshotting.
...
- **Edit metadata.** Owner or any admin can change name, description, slug. Slug must remain unique and not collide with the reserved list. Renames take effect immediately, and the old slug stops resolving.
- **Rotate code.** Owner or admin. Old code dies, pending requests survive (already-in-the-queue requests have been seen and admins decide on them, rotation just stops new ones).
...
- **`/groups/$slug`**: group page. Three stacked sections (no tabs):
  - **Matches** (top, default focus). Two stacked panels: "Members have what you want", then "Members want what you have". Each panel groups by counterparty, with cards rendered via the existing `<CardCell>` slot pattern (per the repo card-browser conventions).
...
- **`/groups/$slug/members/$user_id`**: single-member view inside a group. Shows the member's matched cards (filtered to this counterparty), all of their lists shared with this group grouped by intent (wish / trade / organize, where `organize` lives here as informational, not in the match view), their nickname, and contact-info-as-typed. This is the "drill into one trade partner" page.

### Sharing affordance

List-share toggles live on the group page and on the list's share dialog. From `/lists/$id`, a passive read-only badge says "shared with N groups" and links to a popover that lists the group names. The share dialog (opened from the list's overflow menu) exposes the same per-group checkboxes alongside the public link controls. Both surfaces write to the same `friend_group_list_shares` rows and invalidate each other's caches.
...
- **Avatar-menu badge** (personal): counts pending invites addressed to the viewer (i.e. `direction='invite'` rows with `user_id = me`). Clicking opens the avatar menu, which has an "Invites (N)" item linking to `/groups`.
- **Per-group-row badge** (admin): each group row in `/groups` shows a count of pending requests for that group (`direction='request'` rows where the viewer is admin/owner of the group). Plain members in that group see no badge.
...
- **Trade execution.** No proposals, counter-offers, accept/reject flows, or two-sided ledger linking. The "trade sessions" item deferred in ADR-005 is superseded. We will not build it. Trades happen out-of-band. Each user updates their own collection events afterwards, and the app does not coordinate or verify the exchange.
