...
> **Amendment 5 (2026-08-05).** §II.7 item 3 ruled that no count surface expands, on the grounds that expanding every list on a dashboard is too expensive. That held for the sidebar but not for the share-facing surfaces: a shared dynamic list reading "0 cards" to the person it was shared with is a bug, not a cost saving, so those surfaces started expanding (`expandRuleListCounts`) without the ADR being updated. The rule now splits by surface, and the reason it can is that expanding a _count_ turned out to be far cheaper than expanding a _list_.
>
> 1. **Share-facing surfaces expand.** The group's shares, a member's shares, the shareable-list picker, and the public user-share bundle report the rule-expanded size. `listForUser` — the collections sidebar — still reports the manual count.
> 2. **Counting is not listing.** The first implementation counted by calling `entriesWithDetailsAnon` per list, which enriches every matched entry with card/printing/copy detail and sorts it by name, only for the caller to read `.length`. `listsRepo.expandedCounts(listIds)` stops at `expandList` and returns sizes. It also batches: one query for the lists, one for every list's manual entries, and one inventory read per distinct **owner** rather than per list, so cost scales with owners rather than with lists. Measured on a group of 15 shares carrying 10 rule lists, 84ms / 43 queries became 46ms / 7.
> 3. **Unscoped copies are the batching trade.** The per-list path narrows the inventory load with `ownedCopyPrintingScope`. That scope is a per-rule filter pass, so it can't be shared across lists; the batched path loads each owner's copies whole. One unscoped read beats N narrowed ones at the collection sizes this runs on, and it is the thing to revisit first if that stops being true.
> 4. **Raw entry rows are enough to merge.** The batched path feeds `expandList` straight from `list_entries` instead of the enriched queries. The table's constraints already guarantee what those INNER joins checked: FKs to `cards` / `printings` / `copies`, `fk_list_entries_list_kind` for the entry-to-list kind match, and `chk_list_entries_kind_shape` for exactly one id column being set.
> 5. **The sidebar's rule indicator finally exists.** Item 3 promised one as the compensation for the missing count and it was never built, so a dynamic list showed no count and nothing explaining why. The sidebar row now carries the same sparkle the list page uses for rule-sourced entries, off the `hasRule` flag the summary response already returned.
>
> Still deferred, and still for the original reason: expanding counts in `listForUser`. It would cost roughly +26ms and three extra queries on every sidebar load, and that query refetches on every list mutation, not once a session. What is left after the batching above is the rule evaluation itself, a full-catalog `filterCards` pass per rule that no amount of batching removes — and it is synchronous, so it serializes across concurrent requests instead of overlapping the way the DB waits do. 3. **Personal copies only.** An organize copy list may hold group-shared copies added by hand (`personalOnly = false` on the manual-add routes), but a rule only ever draws on the owner's own copies, exactly like a trade rule — `ownedRowsForUser` is the single copy source on both the server and the editor preview. Widening it would need a group-visibility-aware copy query and a separate decision about what the anonymous public-share path may expand, so it is deferred. 4. **Wording, not math, differs per intent.** The keep/offer split has nothing to offer on an organize list, so the same control reads as "leave out the nicest N per card, include the rest" there (`apps/web/src/lib/rule-wording.ts` holds the per-intent copy, `rulePresetsFor` the per-intent presets). `keepPerCard: 0`, the default, includes every matching copy.

> **Amendment 6 (2026-08-06).** Amendment 3 made `netOwned` count only copies whose printing the rule's filter matched. Price came along as just another filter dimension, and it should not have: a price bound is a budget for what to buy, not a property that makes a copy you already own stop existing. The netting pool now always drops the price range; every other dimension still applies.
>
> 1. **The bug this fixes.** A special version is nearly always dearer than the standard printing, so a cap tuned to the latter excluded exactly the copies "Count special versions" promised to count, defeating the toggle in its main use case. Reported on a list capped at €1.75: two standard copies plus an alt art at €6.25 still asked for a third. Two standard printings of one card hit it too (a rare's normal and foil versions, or a battlefield's standard and oversized), with no alt art in sight. Worse, a printing with no price at all fails any bound outright (`matchesRange` rejects a null value), so those copies were invisible to netting however cheap they were.
> 2. **Netting must not be time-varying.** With price in the pool the same collection netted differently after each price refresh, so a card reappeared on a wishlist with the user having done nothing. What a rule _asks for_ is legitimately time-varying, since that is what a budget means, but what you already own is not.
> 3. **Nothing became inexpressible.** Price is a property of a printing, not of a copy, and netting keys on printing id, so any split a price bound could draw between owned copies is a split between printings, already separable by set, art variant, finish, rarity, signed, markers, language, size, and distribution channel. "My expensive alt art shouldn't count as one of my three playable copies" is `artVariantsExclude`, or standard-only with the toggle off. Only the _dynamic_ form is gone ("whatever is dear right now doesn't count"), which is item 2.
> 4. **Targets and acceptable printings are untouched.** The price bound still decides which cards the list asks for and, through `acceptablePrintingIds`, which printings a group trade may send. So an owned out-of-budget copy satisfies a want while an incoming one does not. That asymmetry is deliberate: the budget governs acquisition, not inventory.
> 5. **Cost is gated.** The relaxed pass is a second full-catalog `filterCards`, the synchronous hot spot Amendment 5 called out, so it only runs when `ruleFiltersOnPrice(rule)`. Otherwise the pool aliases the matched set exactly as before. Where it does run, no dimension reads prices, so the resolver is dropped. `ownedCopyPrintingScope` reads `netPrintings`, so the copy-load narrowing follows with no second site to keep in sync.
>
> Printing-kind lists are unaffected: their target and their pool are the same printing, so price gates both together and no shortfall can survive on a printing the filter rejected.
...
3. **Count surfaces** `lists.listForUser` (:143), `userShares.listsForOwner` (:88), `friendGroups.listShareableForUserInGroup` (:626): do not expand. Return manual `entryCount` + `hasRule`. The UI shows a "rule" indicator. (Avoids expanding every list on dashboards.) **Superseded in part by amendment 5** — the share-facing surfaces expand after all, through a batched count-only path; only `listForUser` still returns the manual count.
...
- **Expanded counts on list summaries:** summaries show manual count + `hasRule`. Full counts appear on detail. (Amendment 5 pulled the share-facing summaries back in; the sidebar's own list summaries still work this way.)
