...
Both copy mutations go through the copies **service** (not the raw `copiesRepo`) so `collection_events` (`added` / `removed`, preserved-on-delete via the `SET NULL` FK from migration 140) and derived counts stay consistent. List-entry quantity changes do not emit collection events, so the wishlist decrement uses the lists repo directly. Never raw SQL (per `docs/contributing.md`). Skipping is always allowed; the trade is "done" regardless, and a party who declines to let the app touch their data simply leaves their lists as-is.
