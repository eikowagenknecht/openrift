...
- Each send path also has an admin kill switch (`trade-request-email`, `trade-match-digest`, `trade-status-email`) so a bug can be stopped without a deploy. They are api-scoped **site settings**, not feature flags: a flag gates a rollout and is deleted once its feature lands, while these are standing operational config that outlives any rollout. Being api-scoped keeps them out of the public `/api/v1/site-settings` payload, which serves the `web` scope only. They are default-on: absent (never created) or any value but `"false"` sends, and setting `"false"` stops it. Checked at the send site as `getBool(key) !== false`, so a key that was never created still sends. Per-user preferences and unsubscribe still apply on top. Migration 243 moved them out of `feature_flags`, carrying each flag's `enabled` state across so a switch someone had already turned off stayed off.
