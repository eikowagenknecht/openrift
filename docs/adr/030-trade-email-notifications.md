...
- Each send path also has an admin kill-switch feature flag (`trade-request-email`, `trade-match-digest`) so a bug can be stopped without a deploy. They are default-on: absent (never seeded) or enabled sends (the ADR behaviour above), and turning the flag off stops it. Checked at the send site as `isEnabled(flag) !== false`. They're seeded `enabled=true` in the admin `KNOWN_FLAGS` so creating one is a no-op (still sends). Only deliberately toggling it off disables the feature. Per-user preferences and unsubscribe still apply on top.
