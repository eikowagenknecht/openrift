...
We reuse the `decks` shape with a synthetic owner plus a satellite `tournaments_decks` table. It inherits the entire deck-rendering, card-list, champion, format, and share-token infrastructure for free, while keeping tournament-specific concerns (event, finish, player) isolated to one satellite table that the rest of the app can ignore.
