...
We build a server-backed pod-tournament runner under a shared `/tournaments` hub (the runner at `/tournaments/run`), owned by an OpenRift account, with free-text players. The pairing engine is a pure local-search module in `packages/shared` behind a strategy interface; an `apps/api` service plus repository loads state, runs the engine, and persists each round in a transaction. A token-gated participant surface offers read-only follow-along plus result entry for any pod in the open round. The organizer finalizes each round, which is the gate that turns drafts into counted results. Standings, opponent history, and pod-size tallies are derived on read from the finalized rounds (the lean model, see "Data model"), so finalize is just a status flip and there is no denormalized state to keep in sync.
...
The token authorizes submitting _any_ pod's result in the open round, not one specific pod (trust-on-link), bounded as described under "Report-link scope." Read-only follow-along plus that single write are the whole of what the token allows.
