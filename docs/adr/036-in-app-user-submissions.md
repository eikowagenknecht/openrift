...
- **Rate limit is the daily cap.** `USER_SUBMISSION_DAILY_LIMIT = 50` is enforced inside the ingest transaction by counting the user's candidate rows in the trailing 24h (`ingestRepo.countRecentSubmissionsByUser`), which is race-safe and survives restarts. A 256 KB Hono `bodyLimit` fronts the route; there is no per-IP burst limiter (the daily cap plus the signed-in requirement are the agreed defense).
