...
- **After submit:** a thank-you confirmation, and nothing further (fire and forget). No status page and no accept/reject notification in v1. The `submitted_by_user_id` column keeps a later status/notification surface a pure addition. (A status page shipped later at `/contribute/submissions`; it needed a second migration after all, because the column recorded who submitted but nothing recorded what happened. See "Outcome ledger" under Implementation Notes.)
...
- **Outcome ledger (added later, migration 234).** The "no status page in v1" choice above assumed `submitted_by_user_id` alone kept a status surface a pure addition. Attribution survived; the outcome did not. Nothing links an accepted candidate to the card it produced, ignoring only writes a `(provider, external_id)` key, and field-level acceptance (`acceptField` / `acceptPrintingField`) carries no candidate id at all. So a `card_submissions` table now records one row per submission: submitter, kind, snapshots of the name and note, a `proposed_diff`, a status, and the contributor-visible reason and note. It sits **beside** the candidate pipeline rather than replacing it, so the "standalone table + bespoke review UI" option rejected above stays rejected: review is still the existing candidate tabs.
  - Status is not on `candidate_cards` because staging is disposable (`deleteByProvider` hard-deletes a provider's rows, and the batch ingest deletes rows absent from a payload) while a contributor's history has to outlive a cleanup. The ledger's `candidate_card_id` is `ON DELETE SET NULL` for the same reason. The per-user daily cap now counts the ledger too, so purging staging can no longer hand a spammer a fresh allowance.
  - **Outcomes derive from the verbs the admin already uses**, not from a new control: check (the review run's "Check all & next") settles a submission once its card and every printing are checked, and ignore rejects it. Field accepts deliberately trigger nothing, since a correction spans several fields and the first accepted one is not the outcome.
  - **What settles it is the catalog, not the click.** The compare grid shows every provider's value side by side, so crediting a contributor by which cell the admin clicked would mark them rejected whenever a scraped provider carried the same correct value. Instead `proposed_diff` records what differed from the live catalog at submission time, and resolution re-runs that comparison: any of those fields the catalog now agrees with means `accepted`, none means `not_applied`, and an empty diff at submit time means `already_correct`.
  - **Rejection keeps its reason.** "Ignore permanently" reads as "Reject submission" on a user-submission column (the key is minted per submission and never re-uploaded, so ignoring only ever means rejecting) and opens a dialog with a required, preselected reason plus optional free text. A separate "Reply to contributor" writes the same message without rejecting, at any point in the submission's life. Unignoring returns the submission to `pending`.
  - Contributors read the result at `/contribute/submissions`.
