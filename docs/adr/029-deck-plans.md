...
- **Matchup plans** (zero or more per deck): the opponent Legend plus a subtitle, matchup notes, and the sideboard swaps (cards in / out).
...
- **Catalog integration is the value-add** (same principle as ADR-014). Every card named in a plan (swapped cards, the opponent Legend, the chosen battlefields) links to its catalog page and can show a thumbnail or ownership overlay.
- **Lightweight taxonomy.** The app deliberately has no archetype/matchup taxonomy (ADR-014 deferred it). We identify an opponent by an existing catalog **Legend** card plus a free-text subtitle, not by a new archetype table.
...
For **opponent identity**: free-text label / pick a champion card / pick a **Legend** card + free-text subtitle / link to another OpenRift deck.
...
- **Matchup plans:** zero or more per deck. Each is identified by a catalog **Legend** card plus an optional free-text subtitle. In Riftbound the Legend is the named identity (e.g. "Diana") and the subtitle captures the build (e.g. "Scorn of the Moon"). The Legend supplies the icon and a catalog link; the subtitle adds the build nuance. No archetype table. One plan per (Legend, subtitle) pair, displayed in a user-controllable order.
...
- Good, because the card-bearing fields are real FKs to `cards`, so every swapped card, opponent Legend, and chosen battlefield links to the catalog and reuses the existing thumbnail, ownership, and card-detail wiring.
...
- Bad, because identifying opponents by a single Legend card is coarser than a real archetype taxonomy; two different decks can share a Legend. The free-text subtitle absorbs this, and matching ADR-014 we choose not to build archetypes yet.
...
create table deck_matchup_plans (
  ...
  opponent_legend_card_id  uuid not null references cards(id),
  subtitle                 text not null default '',   -- free-text build, e.g. "Scorn of the Moon"
  notes                    text not null default '',   -- per-matchup note
  sort_order               smallint not null default 0,
  created_at               timestamptz not null default now(),
  updated_at               timestamptz not null default now(),
  unique (deck_id, opponent_legend_card_id, subtitle)
);
...
- `subtitle` is `NOT NULL DEFAULT ''` rather than nullable so `unique (deck_id, opponent_legend_card_id, subtitle)` actually enforces "one plan per matchup". Postgres treats `NULL` as distinct, which would silently allow duplicate same-Legend plans.
...
- Server-side validation at the API boundary: referenced cards exist, the opponent card is a Legend, the battlefield cards are battlefields, quantities are positive. The balance and availability checks (in vs out counts, copies available, battlefield-in-deck) are advisory and computed client-side against the loaded deck.
...
- Matchups section: add / remove / reorder matchups. Each has an opponent Legend picker (catalog search filtered to Legend cards), a subtitle field, a notes field, and two swap columns. The `out` picker is constrained to the deck's maindeck cards, the `in` picker to its sideboard cards.
...
- **Archetype taxonomy.** No `archetypes` table; opponents are a Legend card plus subtitle, consistent with ADR-014.
...
- Repo + route tests: replace-then-read round-trips a deck's plan (deck-level fields, matchups, swaps); `unique (deck_id, opponent_legend_card_id, subtitle)` rejects a duplicate matchup; deleting a deck cascades the `deck_plans` row, its matchups, and their swaps.
