  ...
  const data = listRows
    ...
    .map((row): ListResponse => {
      ...
      return {
        ...
        // `hasRule` is server-derived too: the lists shape doesn't sync the
        // ADR-034 rules column (rule expansion is server-side anyway).
        hasRule: server?.hasRule ?? false,
...
export function useListDetail(listId: string) {
  ...
  let list: ListDetailListResponse = serverData.list;
  if (syncedRow) {
    list = {
      ...serverData.list,
      name: syncedRow.name,
  ...
  if (entryRows && entriesReady) {
    ...
    entries = serverData.entries.flatMap((entry): ListEntryDetailResponse[] => {
      // ADR-034: rule-produced entries have no list_entries row to sync
      // against (their id is null) — the server expansion is their only
      // source of truth, so they pass through untouched ("both" entries
      // carry the manual row's id and take the synced path below).
      if (entry.source === "rule" || entry.id === null) {
        return [entry];
      }
      ...
      return [
        {
          ...entry,
          // ADR-034 additive model: the synced row carries the manual part
          // of the quantity; the rule's contribution rides on top
          // (ruleQuantity is 0 for purely manual entries).
          quantity: row.quantity + entry.ruleQuantity,
          tradeOverride: tradeOverrideFromEntryRow(row),
        },
      ];
    });
    // Matches the server semantics: entryCount counts list_entries rows
    // (manual entries), never rule-expanded ones.
