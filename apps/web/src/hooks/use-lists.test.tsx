...
describe("useListDetail", () => {
  ...
  function seedDetail(client: QueryClient) {
    const detail: ListDetailResponse = {
      list: {
        ...serverList(LIST_ID, { name: "Wants", entryCount: 2 }),
        rules: [],
        ruleCombine: null,
      },
      entries: [
  ...
  it("passes rule-produced entries through untouched and layers rule quantity onto both-entries", async () => {
    // Regression for the ADR-027 × ADR-034 merge: rule-produced entries have
    // no list_entries row (id is null), so the synced-existence filter must
    // not drop them, and a "both" entry's quantity is manual + ruleQuantity.
    const client = makeClient();
    const detail: ListDetailResponse = {
      list: {
        ...serverList(LIST_ID, { name: "Wants", entryCount: 1, hasRule: true }),
        rules: [],
        ruleCombine: null,
      },
      entries: [
    ...
    client.setQueryData(queryKeys.lists.detail(USER_ID, LIST_ID), detail);
    const entries = createEntriesShape();
    // Only the manual row syncs; its quantity is the manual part (1 of 3).
    entries.insert([makeEntryRow("e1", { quantity: 1 })]);
    collectionMocks.syncedListEntries = entries;
    ...
    expect(result.current.data.entries[1]).toMatchObject({
      id: null,
      source: "rule",
      quantity: 1,
      cardName: "Braum",
    });
    // entryCount counts list_entries rows (manual entries) only.
    expect(result.current.data.list.entryCount).toBe(1);
  });
