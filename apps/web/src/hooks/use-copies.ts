...
// Shape-row defaults for a copy created without metadata (quick-add). The
// columns ride the copies shape (ADR-038), so every optimistic row must be
// complete.
const EMPTY_ROW_METADATA = {
  condition: null,
  grader: null,
  grade: null,
  notes_public: null,
  ...
  is_altered: false,
  links: [],
} satisfies Partial<CopyShapeRow>;

// A camelCase metadata patch (API vocabulary) as shape-row columns. Normalized
// with the same helper the server uses so the optimistic row matches what the
// stream will deliver (e.g. setting a condition clears grading).
function metadataAsRowColumns(patch: CopyMetadataPatch): Partial<CopyShapeRow> {
  const normalized = normalizeCopyMetadataPatch(patch);
  const columns: Partial<CopyShapeRow> = {};
  if (normalized.condition !== undefined) {
    columns.condition = normalized.condition;
  }
  if (normalized.grader !== undefined) {
    columns.grader = normalized.grader;
  }
  if (normalized.grade !== undefined) {
    columns.grade = normalized.grade;
  }
  if (normalized.notesPublic !== undefined) {
    columns.notes_public = normalized.notesPublic;
  }
  if (normalized.notesPrivate !== undefined) {
    columns.notes_private = normalized.notesPrivate;
  }
  if (normalized.isAltered !== undefined) {
    columns.is_altered = normalized.isAltered;
  }
  if (normalized.links !== undefined) {
    columns.links = normalized.links;
  }
  return columns;
}
...
export function useAddCopies() {
  ...
  return useMutation({
    ...
    mutationFn: async (body: {
      copies: ({ printingId: string; collectionId: string; id?: string } & CopyMetadataPatch)[];
      ...
      const rows: CopyShapeRow[] = body.copies.map(
        ({ printingId, collectionId, id, ...metadata }) => ({
          id: id ?? uuidv7(),
          printing_id: printingId,
          collection_id: collectionId,
          // Per-copy metadata (ADR-038), e.g. the CSV import's conditions.
          ...EMPTY_ROW_METADATA,
          ...metadataAsRowColumns(metadata),
        }),
      );
 ...
 * batch of copies, optimistically through the synced copies shape: the patch
 * applies to the local rows instantly, persists to the outbox, and confirms by
 * txid when the change arrives back on the Electric stream. Normalized with
 * the same shared helper the server uses, so the optimistic rows match what
 * the stream will deliver.
 *
...
export function useUpdateCopies() {
  ...
  return useMutation({
    ...
    mutationFn: async ({ copyIds, patch }: { copyIds: string[]; patch: CopyMetadataPatch }) => {
      if (!writer) {
        throw new Error("Cannot update copies while signed out");
      }
      const columns = metadataAsRowColumns(patch);
      if (Object.keys(columns).length === 0 || copyIds.length === 0) {
        return;
      }
      const collection = writer.collection;
      // Rows still in flight from useBatchedAddCopies are real uuids the
      // server may not know yet; the outbox replays FIFO, so their add lands
      // before this patch — no filtering needed.
      const tx = createCopiesTx(writer, "updateCopies");
      tx.mutate(() => {
