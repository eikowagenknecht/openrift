...
export type DeckMetaPartKey = "missing" | "value" | "updated";
...
/**
 * The deck-list stat sequence, in the order the deck hero established: the
 * actionable ownership gap before the price, with the date trailing. Both the
 * grid tile and the list row render this array, so their order cannot drift
 * apart again. The card count is deliberately absent — like the hero, these
 * surfaces carry the build figure on the format badge instead.
 * @returns All three parts in canonical order, with `text` null where the deck
 * has nothing to say (no missing cards, no price, a local deck with no server
 * inventory). Callers drop those parts inline and show a dash in columns.
 */
