// Deck-builder draft, SERVER-deck backend (ADR-027 decks vertical): edits
// operate on a thin draft facade over the synced deck-cards shape. The
// LOCAL-deck backend (ADR-035 browser-local decks) lives in
// deck-builder-collection.ts; hooks/use-deck-builder.ts picks per deck via
// isLocalDeckId. Each edit applies
 ...
 */
export function useSyncedDeckSaveStatus(deckId: string): DeckSaveStatus {
