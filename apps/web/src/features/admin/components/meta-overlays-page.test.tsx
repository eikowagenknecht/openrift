...
vi.mock("@/features/admin/components/meta-event-correction-card", () => ({
  MetaEventCorrectionCard: ({ correction }: { correction: AdminMetaEventCorrection }) => (
    <div>{`correction:${correction.submission.id}`}</div>
  ),
}));
vi.mock("@/features/admin/components/meta-overlay-upload-dialog", () => ({
  MetaOverlayUploadDialog: () => null,
}));
vi.mock("@/features/admin/components/meta-ignored-sources-dialog", () => ({
  MetaIgnoredSourcesDialog: () => null,
}));
vi.mock("@/features/admin/components/meta-card-name-picker", () => ({
  MetaCardNamePicker: ({ name }: { name: string }) => <span>{`picker:${name}`}</span>,
}));
vi.mock("@/features/admin/components/meta-event-search-picker", () => ({
...
vi.mock("@/features/admin/components/meta-standings-row-picker", () => ({
...
const { MetaOverlaysPage } = await import("@/features/admin/components/meta-overlays-page");
