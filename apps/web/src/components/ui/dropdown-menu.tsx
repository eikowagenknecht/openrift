      ...
      >
        <MenuPrimitive.Popup
          ...
          className={cn(
            ...
            // custom: size to content, floored by trigger width and capped to viewport so menus don't shrink to icon triggers or overflow on small screens (replaces upstream w-(--anchor-width) min-w-32)
            "w-max max-w-(--available-width) min-w-(--anchor-width)",
            className,
          )}
          {...props}
        />
...
function DropdownMenuSubContent({
  ...
  return (
    <DropdownMenuContent
      ...
      className={cn(
        // custom: ring-border edge and shadow-md — sub-menus share the parent menu's anchored-popup tier
        "bg-popover text-popover-foreground ring-border data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 w-auto min-w-[96px] rounded-lg p-1 shadow-md ring-1 duration-100",
        className,
      )}
      align={align}
      alignOffset={alignOffset}
...
function DropdownMenuShortcut({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn(
        // custom: text-foreground on highlight — follows the bg-muted menu highlight
