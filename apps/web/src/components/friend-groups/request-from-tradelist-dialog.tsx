...
function RequestBody({
  ...
  const options = wishlistRequestOptions(shareable.items);
  ...
  const [selectedId, setSelectedId] = useState<string>(
    () => preferredWishlistId(options) ?? NEW_LIST,
  );
    ...
    try {
      if (existingMatch) {
        ...
        if (selectedId === NEW_LIST) {
          ...
          listKind = chosen.listKind;
        } else {
          return;
        }
        await bulkAdd.mutateAsync({
          listId,
          entries: [wishEntryForPrinting(listKind, printing, quantity)],
        });
        if (needsShare) {
    ...
    return (
      <>
        ...
        <DialogFooter>
          ...
          <Button type="button" disabled={pending} onClick={sendRequest}>
            Send request
          </Button>
        </DialogFooter>
      </>
