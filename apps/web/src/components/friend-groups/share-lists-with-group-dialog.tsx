  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        >
          <ShareListsBody slug={slug} onOpenChange={onOpenChange} />
        </Suspense>
      </DialogContent>
    </Dialog>
  );
...
function ShareListsBody({
  ...
  // Pre-select every candidate so the common "share them all" case is one click,
  // while still requiring an explicit confirm.
  const [selectedIds, setSelectedIds] = useState<Set<string>>(
    () => new Set(candidates.map((item) => item.listId)),
  );
