...
import type * as UseJobRuns from "@/hooks/use-job-runs";
import type { JobRunsQueryParams } from "@/hooks/use-job-runs";
...
vi.mock("@/routes/_app/_authenticated/admin/job-runs", async () => {
  const { useSyncExternalStore } = await import("react");
  return {
    Route: {
      fullPath: "/admin/job-runs",
      useSearch: () =>
...
vi.mock("@/hooks/use-job-runs", async () => {
  const actual = await vi.importActual<typeof UseJobRuns>("@/hooks/use-job-runs");
  return {
    ...actual,
