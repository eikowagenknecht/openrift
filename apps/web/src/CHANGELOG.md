...
- feat(Cards): **Quick add from the catalog** — press Ctrl+K (Cmd+K on Mac) while browsing the cards page to open the quick-add palette and drop cards straight into your Inbox without leaving the catalog
...
- fix(App): **Crash adding cards on some iOS setups** — adding cards (and the match tracker) no longer fails with a "crypto.randomUUID is not a function" error on older iOS Safari or when the app is opened over a plain http address
- fix(App): **iOS Home Screen safe areas** — when the app is added to the Home Screen on iPhone, content no longer hides behind the status bar and Dynamic Island: the header's blur carries up behind it, the card popup and the slide-out menu now open just below the island instead of under it, and in landscape everything stays clear of the island and the rounded corners
