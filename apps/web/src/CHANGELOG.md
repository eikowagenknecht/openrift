...
- fix(Collection): **Tidier variant menu layout** — the variant-and-collections menu now lines its counts and add/remove buttons into clean columns and sets each variant off as a quiet header, so the list reads in two clear tiers.
