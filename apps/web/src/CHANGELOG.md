...
- fix: A deck-check entry that's already submitted with an issue flagged no longer shows the "Request changes" button, which would have just re-flagged the same issue
...
- feat: When submitting a tournament deck, you now agree (or not) to the organizer publishing your deck list publicly after the event, and within that choose whether your name and Riot ID appear alongside it. Your deck page shows the current choice, judges see it in the checker and can correct it on request, and tournament software can send it along when pushing decklists
...
- fix: Opening a page right after a new release could leave the app stuck on a broken view instead of recovering. The automatic reload now kicks in cleanly and the page comes back on the new version
- fix: When a link sends you to sign in to do something (like claiming a tournament deck) and you choose to create an account instead, you now land back where you were headed after signing up, instead of being dropped on the home page
- fix: Devices that still had an old service worker installed from an earlier version of the app now clean it up automatically, removing leftover cached data and loading the current version directly
- fix: When a page part fails to load right after a new release (because the open tab still references files from the previous version), the app now reloads itself once to pick up the new version instead of showing an error
- fix: The search box on the rules page now has a clear button and shows how many rules match while you type, and the toolbar buttons look the same as everywhere else in the app
...
- fix: Collection statistics no longer count sets that haven't been released yet, so your completion numbers aren't dragged down by preview cards you can't own. A preview set still shows up if you already own cards from it
...
- feat: Each group now opens to an overview with quick links to its trades, shared cards, and members, plus a recent-activity feed showing completed trades, new matches for you, shared lists and collections, and members joining
- feat: A group's suggested trades and your in-progress trades now share one Trades page, replacing the easily-confused "Trading" and "Trades" tabs, and the group's cards and lists live together on a Shared page
...
- fix: Scrolling through long card lists no longer jumps or shifts as cards load in and settle into place. The browser's automatic scroll adjustment was fighting the way the grid renders, and that's now turned off
- fix: When you group by set or rarity, a card that appears in more than one set (or at more than one rarity) now shows up once in each section with the count for just that section. Before, the same owned count showed under every section, so a card you only own in one set looked owned in the others too. This applies on the cards page, in the deck builder, and in your collection, where such cards used to collapse into a single entry
- fix: In your collection, hovering a card you own across several printings now lets the fan-out animation spread past the column edge instead of being clipped, matching how it looks on the cards page
...
- feat: You can now drag the rows in the sidebar to reorder your personal collections and your lists. Hover a row and its icon turns into a grip handle to drag, with the item count staying on the right; the Inbox stays pinned at the top and shared group collections stay alphabetical
...
- feat: On a collection page, the box icon in the toolbar has been replaced with a "Show library" toggle. Turning it on widens the grid to every card in the catalog (unowned cards appear with just a + to add them), while the toolbar, select mode, and bulk actions keep working on owned cards exactly like before
- feat: On a collection page, the count pill on every card now shows the per-printing count in this collection with the per-card total in parens, e.g. "x10 (17)" when you own ten copies of the displayed variant and seven across other variants. Clicking the pill opens the variants + in-your-collections popover on every collection page, not only on the All Cards view
- feat: On a collection page in cards view, clicking a variant in the fan behind the card now pins that variant as the top printing for the rest of your session, so the count and add controls reflect the variant you actually care about
- feat: On a wishlist or tradelist, the "Browse catalog to add to list" button has been replaced with a "Show library" toggle that matches collections. When on, the grid widens to the whole catalog with the cards already on the list showing their quantity strip and the rest showing just a + to add them
...
- fix: On a collection page, the Owned filter (None / Partial Playset / Full Playset / More than Full) now actually narrows the grid based on the copies you hold in this collection. Previously it was wired into the panel but did nothing
...
- fix: In table view, opening a card detail no longer covers part of the table at mid-range viewport widths; the table now scrolls horizontally inside its own column instead
...
- fix: On a collection page, the "owned" count next to each card now reflects copies in that specific collection, with the total across all your collections shown in parentheses when it differs — fixing the table view, which previously showed only the global figure
- fix: The "Search in" panel that opens when you focus the search bar now stretches across the full toolbar on phones, so the field chips no longer wrap onto a second cramped row
...
- fix: The Ctrl+K quick-add palette on a collection page now only suggests cards in the languages you've enabled in your profile preferences, instead of every language
- fix: The Ctrl+K quick-add panel now scrolls to keep the highlighted printing in view as you arrow up and down inside an expanded card
- fix: The Ctrl+K quick-add panel now starts fresh each time you open it, instead of remembering the search and selection from last time
