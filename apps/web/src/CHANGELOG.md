...
- feat: Added a toggle to flip the sort order
- feat: Tap a set header to jump back to the start of that set
- feat: Choose which fields to show on each card — ID, title, type, rarity
...
- feat: Filter by Signed card variant

## 2026-02-20

- fix: Search scope chips appear directly below the search box on mobile
- feat: Filter by card version (Normal, Alt Art, Overnumbered), and search by ID
- feat: Search across name, type, and card text using scope chips
- feat: Card count shows inline in the filter bar
...
- feat: Settings menu, dark mode, and filter controls
- feat: Cards are sorted by ID by default
- feat: Real card images, with a toggle to rotate to landscape
- feat: Switched to official Riftbound card data
