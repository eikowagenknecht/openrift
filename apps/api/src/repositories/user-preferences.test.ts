...
describe("userPreferencesRepo", () => {
  ...
  it("upsert merges visibleFields sub-object", async () => {
    const existing = { visibleFields: { rarity: true, set: true } };
    const db = createMockDb([
      { userId: "u1", data: existing, createdAt: new Date(), updatedAt: new Date() },
    ]);
    const repo = userPreferencesRepo(db);
    const result = await repo.upsert("u1", { visibleFields: { rarity: false } } as any);
    expect(result).toBeDefined();
  });

  it("upsert removes visibleFields sub-key when set to null", async () => {
    const existing = { visibleFields: { rarity: true, set: true } };
    const db = createMockDb([
      { userId: "u1", data: existing, createdAt: new Date(), updatedAt: new Date() },
    ]);
    const repo = userPreferencesRepo(db);
    const result = await repo.upsert("u1", { visibleFields: { rarity: null } } as any);
    expect(result).toBeDefined();
  });

  it("upsert removes visibleFields entirely when all sub-keys removed", async () => {
    const existing = { visibleFields: { rarity: true } };
    const db = createMockDb([
      { userId: "u1", data: existing, createdAt: new Date(), updatedAt: new Date() },
    ]);
    const repo = userPreferencesRepo(db);
  ...
  it("upsert creates visibleFields when none exist", async () => {
    const existing = { theme: "dark" };
    const db = createMockDb([
      { userId: "u1", data: existing, createdAt: new Date(), updatedAt: new Date() },
    ]);
    const repo = userPreferencesRepo(db);
    const result = await repo.upsert("u1", { visibleFields: { rarity: true } } as any);
    expect(result).toBeDefined();
  });

  it("upsert sets visibleFields to null to remove it", async () => {
    const existing = { visibleFields: { rarity: true } };
    const db = createMockDb([
      { userId: "u1", data: existing, createdAt: new Date(), updatedAt: new Date() },
    ]);
    const repo = userPreferencesRepo(db);
    const result = await repo.upsert("u1", { visibleFields: null } as any);
    expect(result).toBeDefined();
  });
});
