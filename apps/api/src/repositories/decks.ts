...
export function decksRepo(db: Kysely<Database>) {
  return {
    ...
    listForUser(userId: string, wantedOnly?: boolean) {
    ...
    getIdAndFormat(id: string, userId: string) {
      return db
        .selectFrom("decks")
        .select(["id", "format"])
        .where("id", "=", id)
    ...
    cardsWithDetails(deckId: string) {
      return db
    ...
    cardRequirements(deckId: string) {
      return db
    ...
    availableCopiesByCard(userId: string) {
      return db
