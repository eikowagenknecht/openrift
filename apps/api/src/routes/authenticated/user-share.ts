...
export const userShareRoute = userShareApp
  ...
  .openapi(getShareState, async (c) => {
    ...
    return c.json({ shareToken: row?.shareToken ?? null } satisfies UserShareStateResponse);
  })
  ...
  .openapi(enableShare, async (c) => {
    ...
    if (current?.shareToken) {
      return c.json({ shareToken: current.shareToken } satisfies UserShareStateResponse);
    }
    const updated = await userShares.setShareToken(userId, generateShareToken());
    assertFound(updated, "User not found");
    return c.json({ shareToken: updated.shareToken } satisfies UserShareStateResponse);
  })
