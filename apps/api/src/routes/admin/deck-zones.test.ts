...
const handler = new OpenAPIHandler(adminDeckZonesRouter, { interceptors: [appErrorInterceptor] });
...
for (const path of [
  "/api/admin/v1/deck-zones",
  "/api/admin/v1/deck-zones/reorder",
  "/api/admin/v1/deck-zones/:slug",
]) {
  app.all(path, handle);
}
