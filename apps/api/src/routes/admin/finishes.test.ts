...
const handler = new OpenAPIHandler(adminFinishesRouter, { interceptors: [appErrorInterceptor] });
const app = new Hono<{ Variables: Variables }>();
app.use("*", async (c, next) => {
  c.set("user", { id: USER_ID } as never);
  c.set("repos", { finishes: mockRepo } as never);
  await next();
});
...
for (const path of [
  ...
  "/api/admin/v1/finishes/reorder",
  "/api/admin/v1/finishes/:slug",
]) {
  app.all(path, handle);
}
