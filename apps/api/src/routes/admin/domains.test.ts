...
import { adminDomainsRouter } from "./domains";
...
const handler = new OpenAPIHandler(adminDomainsRouter, { interceptors: [appErrorInterceptor] });
...
for (const path of [
  "/api/admin/v1/domains",
  "/api/admin/v1/domains/reorder",
  "/api/admin/v1/domains/:slug",
]) {
  app.all(path, handle);
}
