...
import { adminSuperTypesRouter } from "./super-types";
...
const handler = new OpenAPIHandler(adminSuperTypesRouter, { interceptors: [appErrorInterceptor] });
const app = new Hono<{ Variables: Variables }>();
app.use("*", async (c, next) => {
  c.set("user", { id: USER_ID } as never);
  c.set("repos", { superTypes: mockRepo } as never);
  await next();
});
...
for (const path of [
  "/api/admin/v1/super-types",
  "/api/admin/v1/super-types/reorder",
  "/api/admin/v1/super-types/:slug",
]) {
  app.all(path, handle);
}
...
describe("GET /super-types", () => {
  ...
  it("returns 200 with the rows", async () => {
    ...
    const res = await app.request("/api/admin/v1/super-types");
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.superTypes).toHaveLength(2);
    expect(json.superTypes[0].slug).toBe("primary");
  });
  ...
  it("returns an empty array when none exist", async () => {
    ...
    expect(lintBody.superTypes).toEqual([]);
  });
});

describe("PUT /super-types/reorder", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns 204 on a valid reorder", async () => {
    ...
    const res = await app.request("/api/admin/v1/super-types/reorder", {
      method: "PUT",
...
describe("POST /super-types", () => {
  ...
  it("returns 201 when created", async () => {
    ...
    const json = await res.json();
    expect(json.superType.slug).toBe("secondary");
...
describe("PATCH /super-types/:slug", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns 204 when updating the label", async () => {
    ...
    const res = await app.request("/api/admin/v1/super-types/primary", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns 404 when the slug does not exist", async () => {
    mockRepo.getBySlug.mockResolvedValue(undefined);
    const res = await app.request("/api/admin/v1/super-types/missing", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ label: "Renamed" }),
    });
...
describe("DELETE /super-types/:slug", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns 204 when not well-known and not in use", async () => {
    ...
    const res = await app.request("/api/admin/v1/super-types/primary", { method: "DELETE" });
    expect(res.status).toBe(204);
  ...
  it("returns 404 when the slug does not exist", async () => {
    mockRepo.getBySlug.mockResolvedValue(undefined);
    const res = await app.request("/api/admin/v1/super-types/missing", { method: "DELETE" });
    expect(res.status).toBe(404);
    const lintBody = await res.json();
    expect(lintBody.message).toContain("not found");
