...
marketplaceGroupsRoute.use("/admin/marketplace-groups", requireAdmin);
marketplaceGroupsRoute.use("/admin/marketplace-groups/*", requireAdmin);

marketplaceGroupsRoute.get("/admin/marketplace-groups", async (c) => {
...
marketplaceGroupsRoute.patch("/admin/marketplace-groups/:marketplace/:id", async (c) => {
  const marketplace = c.req.param("marketplace");
  const groupId = Number(c.req.param("id"));
  const { name } = updateGroupSchema.parse(await c.req.json());
