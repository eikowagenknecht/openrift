...
const handler = new OpenAPIHandler(adminDeckFormatsRouter, { interceptors: [appErrorInterceptor] });
...
for (const path of [
  "/api/admin/v1/deck-formats",
  "/api/admin/v1/deck-formats/reorder",
  "/api/admin/v1/deck-formats/:slug",
]) {
  app.all(path, handle);
}
