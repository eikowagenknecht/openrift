...
import { adminCardTypesRouter } from "./card-types";
...
const handler = new OpenAPIHandler(adminCardTypesRouter, {
  interceptors: [appErrorInterceptor],
});
const app = new Hono<{ Variables: Variables }>();
app.use("*", async (c, next) => {
  c.set("user", { id: USER_ID } as never);
  c.set("repos", { cardTypes: mockRepo } as never);
  await next();
});
...
for (const path of [
  "/api/admin/v1/card-types",
  "/api/admin/v1/card-types/reorder",
  "/api/admin/v1/card-types/:slug",
]) {
  app.all(path, handle);
}
...
describe("GET /card-types", () => {
  ...
  it("returns 200 with the rows", async () => {
    ...
    const res = await app.request("/api/admin/v1/card-types");
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.cardTypes).toHaveLength(2);
    expect(json.cardTypes[0].slug).toBe("primary");
  });
  ...
  it("returns an empty array when none exist", async () => {
    ...
    expect(lintBody.cardTypes).toEqual([]);
  });
});

describe("PUT /card-types/reorder", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns 204 on a valid reorder", async () => {
    ...
    const res = await app.request("/api/admin/v1/card-types/reorder", {
      method: "PUT",
...
describe("POST /card-types", () => {
  ...
  it("returns 201 when created", async () => {
    ...
    const json = await res.json();
    expect(json.cardType.slug).toBe("secondary");
...
describe("PATCH /card-types/:slug", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns 204 when updating the label", async () => {
    ...
    const res = await app.request("/api/admin/v1/card-types/primary", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns 404 when the slug does not exist", async () => {
    mockRepo.getBySlug.mockResolvedValue(undefined);
    const res = await app.request("/api/admin/v1/card-types/missing", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ label: "Renamed" }),
    });
...
describe("DELETE /card-types/:slug", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns 204 when not well-known and not in use", async () => {
    ...
    const res = await app.request("/api/admin/v1/card-types/primary", { method: "DELETE" });
    expect(res.status).toBe(204);
  ...
  it("returns 404 when the slug does not exist", async () => {
    mockRepo.getBySlug.mockResolvedValue(undefined);
    const res = await app.request("/api/admin/v1/card-types/missing", { method: "DELETE" });
    expect(res.status).toBe(404);
    const lintBody = await res.json();
    expect(lintBody.message).toContain("not found");
