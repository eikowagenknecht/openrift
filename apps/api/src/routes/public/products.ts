...
export const productsRouter = {
  list: os.list.handler(async ({ context }): Promise<ProductsListResponse> => {
    ...
    return { products: products.map((product) => toProductSummary(product)) };
  }),
  ...
  get: os.get.handler(async ({ context, input }): Promise<ProductDetailResponse> => {
    ...
    const contents = await context.repos.products.contents(product.id);
    return { product: toProductSummary(product), contents };
  }),
};
